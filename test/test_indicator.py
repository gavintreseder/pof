...
class TestConditionIndicator(unittest.TestCase):
    ...
    def test_sim_timeline_order_none_then_data(self):
        ...
        for k, v in expected.items():
            ...
            # TODO check why timeline isn't updating
            ci.sim_timeline(*args, **kwargs)
            np.testing.assert_array_equal(ci.get_timeline(
                name), v, err_msg='Failed with %s' % (str(mock.call_args_list[k])))
    ...
    def test_condition_starts_zero(self):
        ci = ConditionIndicator(perfect=0, failed=100,
                                pf_curve='linear', pf_interval=10)
    ...
    def test_condition_starts_positive(self):
        ci = ConditionIndicator(perfect=100, failed=0,
                                pf_curve='linear', pf_interval=10)
        self.assertEqual(ci.get_condition(), 100)
    ...
    def test_sim_timeline_early_start_early_stop(self):
        ...
        ci = ConditionIndicator(perfect=100, failed=50,
                                pf_interval=50, pf_curve='linear')
    ...
    def test_sim_timeline_v_early_start_early_stop(self):
        ci = ConditionIndicator(perfect=100, failed=50,
                                pf_interval=50, pf_curve='linear')
        expected = np.concatenate(
            (np.full(100, 100), np.linspace(100, 90, 11)))
    ...
    def test_sim_timeline_v_early_start_no_stop(self):
        ci = ConditionIndicator(perfect=100, failed=50,
                                pf_interval=50, pf_curve='linear')
        expected = np.concatenate(
            (np.full(100, 100), np.linspace(100, 50, 51)))
    ...
    def test_set_condition_pf_increasing_above_limit(self):
        ci = ConditionIndicator(perfect=50, failed=100,
                                pf_interval=50, pf_curve='linear')
