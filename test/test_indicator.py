...
class TestConditionIndicator(unittest.TestCase):
    ...
    def test_condition_starts_zero(self):
        ci = ConditionIndicator(perfect=0, failed=100, pf_curve='linear')
        self.assertEqual(d.current(), 0)

    def test_condition_starts_zero_does_not_breach_limit(self):
        d = ConditionIndicator(perfect=0, failed=100, pf_curve='linear')
        d.sim(100)
        self.assertEqual(d.current(), 100)

    def test_condition_starts_positive(self):
        d = ConditionIndicator(perfect=100, failed=0, pf_curve='linear', pf_curve_params =[-10])
        self.assertEqual(d.current(), 100)
    ...
    def test_get_condition_profile_early_start_early_stop(self):
        c = ConditionIndicator(100,50,'linear',[-1])
    ...
    def test_reset(self):
        c = ConditionIndicator(100,0,'linear', [-10], decreasing = True)
        c.sim(10)
        c.reset()
